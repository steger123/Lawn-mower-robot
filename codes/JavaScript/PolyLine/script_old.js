...
var polyline = new L.Polyline([marker_pos[0], marker_pos[1], marker_pos[2]]).addTo(map);  // make the polyline as well
...
function dragStartHandler(e) {
    var latlngPoly = polyline3.getLatLngs(),		// Get the polyline's latlngs
        latlngMarker = this.getLatLng();		// Get the actual, cliked marker's start latlng
    console.log(latlngPoly);
    ...
    for (var i = 0; i < latlngPoly.length; i++) {		// Iterate the polyline's latlngs
        if (latlngMarker.equals(latlngPoly[i])) {		// Compare marker's latlng ot the each polylines 
            this.polylineLatlng = i;			// If equals store key in marker's instance
        }
    }
}
...
function dragHandler(e) {
    ...
    polyline3.setLatLngs(latlngPoly);			// Update the polyline with the new latlngs
}
...
map.on("click", function (e) {  //Listener: Click on MAP -> Addign the WAYPOINTS
    if (doRouting) {
        ...
        new_marker[lineCount] = new L.marker([e.latlng.lat, e.latlng.lng], { icon: locationIcon, draggable: true, opacity: 0.4 }).addTo(map);

        new_marker[lineCount]
            .on('dragstart', dragStartHandler)
            .on('drag', dragHandler)
            .on('dragend', dragEndHandler);
