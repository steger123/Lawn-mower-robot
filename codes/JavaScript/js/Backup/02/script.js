  ...
  document.getElementById("routing-panel").style.display = "block";
    ...
    if (doRouting) {
      ...
      //  property.style.backgroundColor = "#ecebeb"
        ...
        var newPoly2 = new Array(lineCount + 1);  // this is required, becuse newPoly have empty values,
													// which is not accepted by L.Polyline
        // Loop to create 2D array using 1D array 
        for (var i = 0; i < newPoly2.length; i++) {
            newPoly2[i] = new Array(2);   // just a method to make 2D array in JS
...
function update_table(tableID) {
    ...
    // newPoly[lineCount][1] = startPoint[1];
			...
			for(var i=1; i<totalRows; i++) {
				...
				var cell2 = row.insertCell(0);  //waypoint
					cell2.innerHTML = i;
				var latit = newPoly[i][0];		//ontainer of the waipoint's polyline start &next is the end
				var longit = newPoly[i][1];
